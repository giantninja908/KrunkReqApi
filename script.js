...
import {  } from 'https://threejs.org/build/three.js'
...
const interpSkin = (skin) => { //have fun interpreting this!
  ...
  const texUrl = (a['item']['preview'].includes('spray') ? a['item']['preview'] : 'https://assets.krunker.io/textures'+a['item']['preview'].split("previews")[1].replace("weapons/",(a['item']['id'] ? "weapons/skins/" : 'weapons/')).replace(/_m._/,"_").replace(/_m/,'_').replace("cosmetics/1_","hats/hat_").replace("cosmetics/2_","body/body_").replace("cosmetics/1_","hats/hat_").replace("cosmetics/7_","faces/face_").replace("cosmetics/6_","waist/waist_").replace(/weapon_._c/,"pat/")).replace("cosmetics/9_","pets/pet_")
...
const main = (data) => {
  ...
  const funco = () => {
    const res = fuse.search(document.getElementById("skin").value).filter((a,b) => b<=10)

    let da = '';
    res.slice(1).forEach((a) => {
      

      console.log(a['item'])

      const [modelUrl, previewUrl, textureUrl, texUrl, em] = interpSkin(a['item'])
      

      da+="<div class='itema'><p class='name'>"+a.item.name+"<br><span class='auth'> by "+a.item.creator+"</span></p><a class='prev' target='_blank' href='"+modelUrl+"'><img src='"+(a['item']['preview'])+"'/></a><a class='diffuse' target='_blank' href='"+texUrl+"'><img src='"+texUrl+"'/></a><a class='emiss' target='_blank' href='"+em+"'><img src='"+em+"'/></a><div class='infoHolder'>ID: "+ a.item.id +"</div></div>"
    })
